...
export class PerfilValidation_AdministradorSistema implements CanActivate {
    constructor(
    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        ...
        return this.usuarioService.recuperaUsuarioLogado().then(x => {
            ...
            if (x == null && !Config.RecuperaInstancia().recuperaUsuario()) {
               ...
               if(!perfisUsuario ||  perfisUsuario.filter(x=>{return x == Constants.PerfilUsuario.AdministradorSistema}).length == 0){
